...
ggplot(Disease1, aes(x = reorder(disease, number) , y = reorder(number, disease),  fill= event))+
  ...
  theme(text = element_text(size = 6),element_line(size =1),legend.position = c(0.9, 0.2),axis.text.x = element_text(angle = 90, hjust = 1)) +
...
ggplot(State_death_count, aes(state,disease)) +
  ...
  scale_fill_continuous(high= "darkblue",low = "grey87") + # Box color low to high
...
ggplot(State_cases_count, aes(state, disease )) +
  ...
  ggtitle("Heatmap of Cases of in State" ) # plot title
